...
def split_by_image(df: pd.DataFrame, val_frac=0.1, test_frac=0.1, seed=42):
    ...
    assert len(train_images.intersection(val_images)) == 0, (
        "Leakage between train and val splits!"
    )
...
def get_dataloaders(
    ...
    train_dataset = CaptionDataset(
        train_df, image_dir, vocab, transform=get_transform(train=True)
    )
    val_dataset = CaptionDataset(
        val_df, image_dir, vocab, transform=get_transform(train=False)
    )
    test_dataset = CaptionDataset(
        test_df, image_dir, vocab, transform=get_transform(train=False)
    )
